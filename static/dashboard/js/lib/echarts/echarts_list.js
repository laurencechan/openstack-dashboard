$.fn.chartsObj = function (option) {
  var setting = {
    ...
    jsonSeriesListData: null         //需要额外添加的seriesList的JSON数据
  };
  ...
  var jsonSeriesListData = option.jsonSeriesListData;

  // change by Siven.
  // 报表json出错或者json数据为空时,显示默认图片
  $.dataUnLoad = function (){
    $(ID).css("background","url(../../demo/skin/blue/images/public/data_null.png) center no-repeat")
    return false;
  }
  ...
  switch (option.isWhat) { //通过isWhat参数判断是什么类型的报表
    ...
    case 'pie1'://实心饼图
      ...
      $.getJSON(jsonA, function (jsonArray) {
        ...
        if (jsonSeriesListData) {
          jsonArray.seriesList = jsonArray.seriesList.concat(jsonSeriesListData)
        }
        ...
        if($.ajaxSettings.async){
          pieEchart(tp, ID, Tg, ou, legend, titleName, seriesData, showTitle, Url)
        }
      });
...
function lineOrbar(tp, ID, Tg, ou, titleName, xAxisData, seriesData, tt, Url) {
  ...
  if(seriesData[0].hasOwnProperty("data")){
